...
function validateCustomerGuidanceOutput(output, expected) {
  ...
  for(let i=0;i<reports.length;i++){
    ...
    const p=find(/^◇ 발행사:/), d=find(/^◇ 발행일:/), u=find(/^자세한 내용의 링크:\s*https?:\/\//), t=find(/^목차:$/), info=find(/^보고서 정보:\s*/);
    ...
    if(info>=0){
      const actual=block[info].trim().replace(/^보고서 정보:\s*/,"");
      if(r.report_info_ko_exact!==undefined&&actual!==r.report_info_ko_exact) errors.push(`REPORT_${n}_REPORT_INFO_EXACT_MISMATCH`);
      if(r.report_info_ko_exact===undefined&&!/\d/.test(actual)) errors.push(`REPORT_${n}_REPORT_INFO_NUMERIC_SOURCE_MISSING`);
    }
