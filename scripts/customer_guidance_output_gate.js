...
function validateCustomerGuidanceOutput(output, expected){
  ...
  reports.forEach((r,i)=>{
    const n=i+1;
    if(r.toc_source_verified!==true) holds.push(`REPORT_${n}_TOC_SOURCE_NOT_VERIFIED`);
    if(!Array.isArray(r.toc_lines)||!r.toc_lines.length) holds.push(`REPORT_${n}_PUBLIC_TOC_EXPECTED_MISSING`);
    if(!r.english_title) holds.push(`REPORT_${n}_OFFICIAL_TITLE_MISSING`);
    if(!r.publisher) holds.push(`REPORT_${n}_PUBLISHER_MISSING`);
    if(!r.url) holds.push(`REPORT_${n}_URL_MISSING`);
  });
  ...
  if(starts.some(x=>x<0)||!(starts[0]<starts[1]&&starts[1]<starts[2])) errors.push("ACTUAL_REPORT_COUNT_NOT_THREE");
  const allReportHeaders=lines.filter(x=>/^추천자료\s+\d+\s*$/.test(x.trim()));
  if(allReportHeaders.length!==3) errors.push("ACTUAL_REPORT_COUNT_NOT_THREE");

  for(let i=0;i<reports.length;i++){
    const n=i+1,r=reports[i]||{},start=starts[i],next=i<2&&starts[i+1]>=0?starts[i+1]:lines.length;
    if(start<0) continue;
    const block=lines.slice(start,next),find=rx=>block.findIndex(x=>rx.test(x.trim()));
    const e=find(/^##\s+영문 제목:\s*\S/),k=find(/^##\s+한글 제목(?:\(참고 번역\))?:\s*\S/),p=find(/^◇ 발행사:/),d=find(/^◇ 발행일:/),u=find(/^자세한 내용의 링크:\s*https?:\/\//),t=find(/^목차:$/),info=find(/^보고서 정보:\s*$/);
    ...
    if(info<0) errors.push(`REPORT_${n}_REPORT_INFO_LABEL_LINE_INVALID`);
    ...
    if(e>=0){const v=block[e].trim().replace(/^##\s+영문 제목:\s*/,"");if(v!==r.english_title) errors.push(`REPORT_${n}_ENGLISH_TITLE_MISMATCH`);}
    if(k>=0&&r.korean_title){const v=block[k].trim().replace(/^##\s+한글 제목(?:\(참고 번역\))?:\s*/,"");if(v!==r.korean_title) errors.push(`REPORT_${n}_KOREAN_TITLE_MISMATCH`);}
    if(p>=0){const v=block[p].trim();if(r.publisher&&!v.includes(`◇ 발행사: ${r.publisher}`)) errors.push(`REPORT_${n}_PUBLISHER_MISMATCH`);if(r.pages&&!v.includes(`( ${r.pages} Pages )`)&&!v.includes(`(${r.pages} Pages)`)) errors.push(`REPORT_${n}_PAGES_MISMATCH`);if(r.price_display&&!v.includes(`◇ 정가: ${r.price_display}`)) errors.push(`REPORT_${n}_PRICE_MISMATCH`);if(!/\(\s*[^)]*Pages\s*\).*◇ 정가:/.test(v)) errors.push(`REPORT_${n}_PUBLISHER_PRICE_LINE_FORMAT_INVALID`);}
    if(d>=0){const v=block[d].trim();if(r.publication_date_display&&!v.includes(`◇ 발행일: ${r.publication_date_display}`)) errors.push(`REPORT_${n}_PUBLICATION_DATE_MISMATCH`);if(!/-PDF-.*◆ 공급가격:/.test(v)) errors.push(`REPORT_${n}_PUBLISHED_SUPPLY_LINE_FORMAT_INVALID`);}
    if(u>=0){const url=block[u].trim().replace(/^자세한 내용의 링크:\s*/,"");if(url!==r.url) errors.push(`REPORT_${n}_DETAIL_LINK_MISMATCH`);if(/[?&](?:utm_[^=]+|fbclid|gclid|mc_cid|mc_eid|_ga|_gl)=|chatgpt\.com/i.test(url)) errors.push(`REPORT_${n}_TRACKING_LINK_FORBIDDEN`);if(/\.pdf(?:$|[?#])|brochure|\/file\//i.test(url)) errors.push(`REPORT_${n}_BROCHURE_OR_FILE_LINK_FORBIDDEN`);}

    const expectedToc=(r.toc_lines||[]).map(x=>String(x).replace(/\s+$/, ""));
    if(t>=0&&info>t&&expectedToc.length){
      const actualToc=block.slice(t+1,info).filter(x=>x.trim()).map(x=>x.replace(/\s+$/, ""));
      ...
      if(actualToc.length&&actualToc.at(-1).trim()!==expectedToc.at(-1).trim()) errors.push(`REPORT_${n}_TOC_LAST_ITEM_MISSING_OR_MISMATCH`);
      for(let j=0;j<Math.min(actualToc.length,expectedToc.length);j++){
        const expNum=(expectedToc[j].trim().match(/^(\d+(?:\.\d+)*\.?|Chapter-\s*\d+)/i)||[,""])[1];
        const actNum=(actualToc[j].trim().match(/^(\d+(?:\.\d+)*\.?|Chapter-\s*\d+)/i)||[,""])[1];
        if(expNum&&actNum!==expNum){errors.push(`REPORT_${n}_TOC_NUMBER_NOT_COPYABLE_OR_MISSING`);break;}
    ...
    if(info>=0){
      const infoBody=(block[info+1]||"").trim();
      if(!infoBody) errors.push(`REPORT_${n}_REPORT_INFO_BODY_MISSING`);
      if(r.report_info_ko_exact!==undefined&&infoBody!==r.report_info_ko_exact) errors.push(`REPORT_${n}_REPORT_INFO_EXACT_MISMATCH`);
      if(block[info].trim()!=="보고서 정보:") errors.push(`REPORT_${n}_REPORT_INFO_LABEL_LINE_INVALID`);
    }
  }
...
function runSelfTest(){
  ...
  const bad=validateCustomerGuidanceOutput(good.replace("추천자료 3","추천자료 X"),e);if(bad.status!=="FAIL")throw new Error("REPORT_COUNT_GATE_FAILED");
  const tracking=validateCustomerGuidanceOutput(good.replace("https://publisher.example/1","https://publisher.example/1?utm_source=chatgpt.com"),e);if(tracking.status!=="FAIL")throw new Error("TRACKING_GATE_FAILED");
  const hold=JSON.parse(JSON.stringify(e));hold.reports[0].toc_source_verified=false;if(validateCustomerGuidanceOutput(good,hold).status!=="HOLD")throw new Error("TOC_SOURCE_HOLD_FAILED");
